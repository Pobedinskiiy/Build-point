 ...
 */
 ...
 class SliderEmployees {
  // базовые классы и селекторы
  static PREFIX = 'slider-employees';
  static EL_WRAPPER = `${SliderEmployees.PREFIX}-container`;
  static EL_ITEM = `${SliderEmployees.PREFIX}-item`;
  static EL_ITEM_ACTIVE = `${SliderEmployees.PREFIX}-item-active`;
  static EL_ITEMS = `${SliderEmployees.PREFIX}-items`;
  static EL_INDICATOR = `${SliderEmployees.PREFIX}-indicator`;
  static EL_INDICATOR_ACTIVE = `${SliderEmployees.PREFIX}-indicator-active`;
  static EL_INDICATORS = `${SliderEmployees.PREFIX}-indicators`;
  static EL_CONTROL = `${SliderEmployees.PREFIX}-btn`;
  ...
  constructor(target, config) {
    ...
    if (this._config.loop) {
      ...
      this._elListItem[count].style.transform = `translate3D(${valueX*(1/this._config.size_slider)}px, 0px, 0.1px)`;
    }
  ...
  _move() {
    ...
    if (this._direction === 'none') {
      ...
      this._elItems.style.transform = `translate3D(${valueX*(1/this._config.size_slider)}px, 0px, 0.1px)`;
      return;
    }
    if (!this._config.loop) {
  ...
  _balanceItems() {
    ...
    if (this._direction === 'next') {
      ...
      if (exItemRight < this._clientRect.left - this._clientRect.width / 2) {
        ...
        this._exItemMin.style.transform = `translate3D(${valueX*(1/this._config.size_slider)}px, 0px, 0.1px)`;
        this._updateExProperties();
      }
    } else {
      ...
      if (exItemLeft > this._clientRect.right + this._clientRect.width / 2) {
        ...
        this._exItemMax.style.transform = `translate3D(${valueX*(1/this._config.size_slider)}px, 0px, 0.1px)`;
        this._updateExProperties();
      }
    }
    window.setTimeout(() => {
  ...
  reset(newValueX = 0, recalc = false) {
    this._autoplay('stop');
    this._elItems.classList.add(this.constructor.TRANSITION_NONE);
    this._elItems.style.transform = `translate3D(${newValueX*(1/this._config.size_slider)}px, 0px, 0.1px)`;
    this._elListItem.forEach((el) => {
      const valueX = recalc ? Number(el.dataset.translate) * this._clientRect.width : 0;
      el.style.transform = `translate3D(${valueX*(1/this._config.size_slider)}px, 0px, 0.1px)`;
    });
    if (!recalc) {
      this._transform = 0;
      this._states = [];
