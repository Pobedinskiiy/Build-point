 ...
 */
 ...
 class SliderProjects {
  ...
  static PREFIX = 'slider-projects';
  static EL_WRAPPER = `${SliderProjects.PREFIX}-container`;
  static EL_ITEM = `${SliderProjects.PREFIX}-item`;
  static EL_ITEM_ACTIVE = `${SliderProjects.PREFIX}__item_active`;
  static EL_ITEMS = `${SliderProjects.PREFIX}-items`;
  static EL_INDICATOR = `${SliderProjects.PREFIX}-indicator`;
  static EL_INDICATOR_ACTIVE = `${SliderProjects.PREFIX}__indicator_active`;
  static EL_INDICATORS = `${SliderProjects.PREFIX}__indicators`;
  static EL_CONTROL = `${SliderProjects.PREFIX}-btn`;
  // порог для переключения слайда (20%)
  static SWIPE_THRESHOLD = 10;
  ...
  constructor(target, config) {
    ...
    if (this._config.loop) {
      ...
      this._elListItem[count].style.transform = `translate3D(${valueX*0.33333333}px, 0px, 0.1px)`;
    }
  ...
  _move() {
    ...
    if (this._direction === 'none') {
      ...
      this._elItems.style.transform = `translate3D(${valueX*0.33333333}px, 0px, 0.1px)`;
      return;
    }
    if (!this._config.loop) {
  ...
  _balanceItems() {
    ...
    if (this._direction === 'next') {
      ...
      if (exItemRight < this._clientRect.left - this._clientRect.width / 2) {
        ...
        this._exItemMin.style.transform = `translate3D(${valueX*0.33333333}px, 0px, 0.1px)`;
        this._updateExProperties();
      }
    } else {
      ...
      if (exItemLeft > this._clientRect.right + this._clientRect.width / 2) {
        ...
        this._exItemMax.style.transform = `translate3D(${valueX*0.33333333}px, 0px, 0.1px)`;
        this._updateExProperties();
      }
    }
    window.setTimeout(() => {
  ...
  reset(newValueX = 0, recalc = false) {
    this._elItems.classList.add(this.constructor.TRANSITION_NONE);
    this._elItems.style.transform = `translate3D(${newValueX*0.33333333}px, 0px, 0.1px)`;
    this._elListItem.forEach((el) => {
      const valueX = recalc ? Number(el.dataset.translate) * this._clientRect.width : 0;
      el.style.transform = `translate3D(${valueX*0.33333333}px, 0px, 0.1px)`;
    });
    if (!recalc) {
      this._transform = 0;
      this._states = [];
